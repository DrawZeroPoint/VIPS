...
def build_target_likelihood_planar_n_link_4(num_dimensions, prior_variance, likelihood_variance):
    ...
    def target_lnpdf(theta, without_prior=False):
        ...
        likelihood = np.max(np.vstack((likelihood1.logpdf(np.vstack((x, y)).transpose()),
                                       likelihood2.logpdf(np.vstack((x, y)).transpose()),
                                       likelihood3.logpdf(np.vstack((x, y)).transpose()),
                                       likelihood4.logpdf(np.vstack((x, y)).transpose()))), axis=0)
        if without_prior:
...
def build_target_likelihood_planar_n_link_4_autograd(num_dimensions, prior_variance, likelihood_variance):
    ...
    def target_lnpdf(theta):
        ...
        for i in range(0, num_dimensions):
            ...
            x += l[i] * np.cos(np.sum(theta[:, :i + 1], 1))
        likelihood = np.max((
            normal_auto.logpdf(np.vstack((x, y)).transpose(), [0.7 * num_dimensions, 0],
                               likelihood_variance * np.eye(2)),
            normal_auto.logpdf(np.vstack((x, y)).transpose(), [-0.7 * num_dimensions, 0],
                               likelihood_variance * np.eye(2)),
            normal_auto.logpdf(np.vstack((x, y)).transpose(), [0, 0.7 * num_dimensions],
                               likelihood_variance * np.eye(2)),
            normal_auto.logpdf(np.vstack((x, y)).transpose(), [0, -0.7 * num_dimensions],
                               likelihood_variance * np.eye(2))))
        return np.squeeze(
            normal_auto.logpdf(theta, np.zeros(num_dimensions), prior_variance * np.eye(num_dimensions)) + likelihood)
...
def build_target_likelihood_planar_n_link_3(num_dimensions, prior_variance, likelihood_variance):
    ...
    def target_lnpdf(theta, without_prior=False):
        ...
        likelihood = np.max(np.vstack((likelihood1.logpdf(np.vstack((x, y)).transpose()),
                                       likelihood2.logpdf(np.vstack((x, y)).transpose()),
                                       likelihood3.logpdf(np.vstack((x, y)).transpose()))), axis=0)
        if without_prior:
            return np.squeeze(likelihood)
        else:
            return np.squeeze(prior.logpdf(theta) + likelihood)
...
### Goodwin Oscillator
def build_Goodwin(target_labels, sigma=0.1, steps=81, deltaS=1.0, startTimeToObserv=41, nosOfObserv=2, gamma_shape=2.,
                  gamma_rate=1., parameters=None, seed=None):
...
def build_Goodwin_grad_with_lnpdf(target_labels, sigma=0.1, steps=81, deltaS=1.0, startTimeToObserv=41, nosOfObserv=2,
                                  gamma_shape=2., gamma_rate=1., parameters=None, seed=None):
    from experiments.lnpdfs.goodwinoscillator.GoodwinOscillator import GoodwinOscillator as GoodwinOscillator
    if parameters is None:
        a1 = 1.0
        a2 = 3.0
...
def build_Goodwin_grad(target_labels, sigma=0.1, steps=81, deltaS=1.0, startTimeToObserv=41, nosOfObserv=2,
                       gamma_shape=2., gamma_rate=1., parameters=None, seed=None):
    from experiments.lnpdfs.goodwinoscillator.GoodwinOscillator import GoodwinOscillator as GoodwinOscillator
    if parameters is None:
        a1 = 1.0
...
def build_ball_in_a_cup_lnpdf_parallel(poolsize, prior_var=2):
    ...
    def referenceTrajectory(theta):
        ...
        for i in range(0, timesteps - 1):
            ...
            acc = dmpAlphaX * (dmpBetaX * (movingGoal - referencePos[i, :]) * tau ** 2 + (
                        goalVel - referenceVel[i, :]) * tau) + \
